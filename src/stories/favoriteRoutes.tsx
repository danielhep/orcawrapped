...
function FavoriteRoutes ({ state }: { state: AppState }): JSX.Element {
  ...
  return (
    <div style={{ aspectRatio: '9/16', boxShadow: '0px 0px 15px 0px #c745ff', borderRadius: 15 }}>
      <ParentSize>
        {
          (parent) => {
            ...
            return (
              <svg width={parent.width} height={parent.height}>
                <LinearGradient id='gradient' from='#351CAB' to='#621A61' rotate='-45' />
                <LinearGradient id='barGradient' from='#351CAB' to='#621A61' rotate='45' />
                <rect rx={15} width={parent.width} height={parent.height} fill="url('#gradient')" />
                <text
                  textAnchor='middle'
                  dominantBaseline='middle'
                  y={30} x={parent.width / 2}
                  ...
                  {data.map((line, i) => {
                    ...
                    return (
                      <React.Fragment key={line.line}>
                        <Bar key={`bar-${line.line}`} width={barWidth} height={barHeight} y={barY} x={0} fill='url(#barGradient)' opacity={0.5} />
                        <text dominantBaseline='middle' y={yScale(line.line)} x={10} style={{ fill: 'white' }}>{line.line}</text>
                      </React.Fragment>
                    )
                  })}
                </Group>
